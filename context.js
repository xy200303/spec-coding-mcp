...
function keywordScore(file, keywords) {
...
function candidateAreaFromFile(file) {
    const lower = file.toLowerCase();
    if (/(test|spec|__tests__|e2e)/.test(lower))
        return "tests";
    if (/(route|controller|api|endpoint|handler|resolver|server)/.test(lower))
        return "backend/API";
    if (/(component|page|screen|view|ui|frontend|client|web|ios|android)/.test(lower))
        return "frontend/client";
    if (/(schema|model|migration|repository|entity|dao|prisma|database|db)/.test(lower))
        return "data/database";
    return "domain/application code";
}
async function findCandidateFiles(root, keywords, limit) {
...
function collectKeywords(changes) {
...
function parentBlocksForChange(change, allBlocks, maxChars) {
    if (!change.block)
        return [];
    const block = change.block;
    return allBlocks
        .filter((candidate) => {
        if (candidate.file !== block.file)
            return false;
        if (candidate.startLine >= block.startLine)
            return false;
        if (candidate.endLine < block.endLine)
            return false;
...
function renderContextMarkdown(context, planMarkdown) {
...
export async function createModelContext(input) {
    ...
    const nearbyContext = unique(actionable.flatMap((change) => parentBlocksForChange(change, scan.blocks, Math.floor(maxBlockChars / 2))).map((item) => JSON.stringify(item))).map((item) => JSON.parse(item));
