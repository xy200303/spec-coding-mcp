...
function blockLabel(change: BlockChange): string {
  ...
  if (block) return `${block.file}:${block.startLine} ${block.titlePath.join(" > ")}`;
  if (previous) return `${previous.file}:${previous.startLine} ${previous.titlePath.join(" > ")}`;
  return "unknown";
}

function suggestFilePatterns(change: BlockChange): string[] {
  const block = change.block;
...
export function createImplementationTask(scan: ScanResult): ImplementationTask {
...
export function renderTaskMarkdown(task: ImplementationTask): string {
