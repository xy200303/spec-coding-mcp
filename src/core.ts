...
export async function scanProject(projectRoot: string, docsDir = "docs"): Promise<ScanResult> {
  const root = path.resolve(projectRoot);
  const blocks = await scanDocs(root, docsDir);
...
export async function planImplementation(projectRoot: string, docsDir = "docs", writePlan = true): Promise<Record<string, unknown>> {
...
export async function markProjectSynced(projectRoot: string, docsDir = "docs"): Promise<Record<string, unknown>> {
  const root = path.resolve(projectRoot);
  const blocks = await scanDocs(root, docsDir);
...
export async function resetState(projectRoot: string, docsDir = "docs"): Promise<Record<string, unknown>> {
  const root = path.resolve(projectRoot);
  const blocks = await scanDocs(root, docsDir);
  const state = await baselineState(root, blocks, docsDir);
