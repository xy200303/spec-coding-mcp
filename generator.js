...
function inferProjectName(projectRoot, explicit) {
    if (explicit?.trim())
        return explicit.trim();
    return path.basename(projectRoot) || "未命名系统";
}
function inferFeatureCandidates(prompt, projectName) {
...
async function writeFileIfChanged(root, relativeFile, content, overwrite, files) {
...
function docsReadme(projectName) {
    return [
        `# ${projectName} 规格文档`,
        "",
...
function systemArchitecture(projectName, source) {
...
function glossary(projectName, terms) {
...
function indexContent(features) {
    return [
        "# 功能索引",
        "",
        "| 功能 | 文档 | 来源 | 状态 |",
...
async function writeBaseDocs(root, docsDir, projectName, overwrite, files, source) {
    const base = docsDir;
...
export async function generateSpecsFromPrompt(input) {
    ...
    for (const feature of features) {
        ...
        await writeFileIfChanged(root, relativeFile, featureDocFromPrompt({ ...feature, projectName, prompt: input.prompt }), overwrite, files);
    }
    await writeFileIfChanged(root, path.join(docsDir, "indexes", "feature-index.md"), indexContent(features.map((feature) => ({
        title: feature.title,
...
function shouldIncludeSourceFile(relativeFile, includePatterns, excludePatterns) {
    const posix = relativeFile.replace(/\\/g, "/");
    if (excludePatterns.some((pattern) => posix.includes(pattern.replace(/\\/g, "/"))))
        return false;
    if (includePatterns.length === 0)
        return true;
    return includePatterns.some((pattern) => posix.includes(pattern.replace(/\\/g, "/")));
}
function classifyFile(relativeFile) {
    const lower = relativeFile.toLowerCase().replace(/\\/g, "/");
    const base = path.basename(lower);
    if (MANIFEST_NAMES.has(base))
        return "manifest";
    if (/(^|\/)(__tests__|tests?|specs?|e2e)(\/|$)|\.(test|spec)\./i.test(lower))
        return "test";
    if (/(route|routes|controller|controllers|api|endpoint|server|handler|resolver)/i.test(lower))
        return "api";
    if (/(component|components|page|pages|screen|screens|view|views|app\/|ui\/)|\.(tsx|jsx|vue|svelte)$/i.test(lower))
        return "ui";
    if (/(schema|model|models|migration|migrations|repository|repositories|entity|entities|dao|prisma|database|db\/)/i.test(lower))
        return "data";
    if (CODE_EXTENSIONS.has(path.extname(lower)))
        return "code";
    return "other";
}
function routeHintsFromContent(relativeFile, content) {
    const hints = new Set();
    const routeRegexes = [
...
function componentHintsFromContent(relativeFile, content) {
    const hints = new Set();
    const regexes = [
...
function modelHintsFromContent(relativeFile, content) {
    const hints = new Set();
    const regexes = [
...
async function scanSource(root, includePatterns, excludePatterns, maxFiles) {
    ...
    for (const absolute of files) {
        ...
        if (kind === "manifest")
            summary.manifests.push(relative);
        if (kind === "api")
            summary.apiFiles.push(relative);
        if (kind === "ui")
            summary.uiFiles.push(relative);
        if (kind === "data")
            summary.dataFiles.push(relative);
        if (kind === "test")
            summary.testFiles.push(relative);
        if (["api", "ui", "data", "code"].includes(kind)) {
            ...
            if (kind === "api" || kind === "code")
                summary.routeHints.push(...routeHintsFromContent(relative, content));
            if (kind === "ui" || kind === "code")
                summary.componentHints.push(...componentHintsFromContent(relative, content));
            if (kind === "data" || kind === "code")
                summary.modelHints.push(...modelHintsFromContent(relative, content));
        }
    }
...
function domainFromFile(file, fallback) {
...
function featureCandidatesFromSource(summary) {
    const candidates = [];
...
function sourceOverview(summary) {
...
export async function generateSpecsFromSource(input) {
    ...
    for (const feature of features) {
        const relativeFile = path.join(docsDir, "features", feature.domain, `${feature.name}.md`);
        featureDocs.push(relativePosix(root, path.join(root, relativeFile)));
        await writeFileIfChanged(root, relativeFile, featureDocFromSource({ ...feature, projectName }), overwrite, files);
    }
    await writeFileIfChanged(root, path.join(docsDir, "indexes", "feature-index.md"), indexContent(features.map((feature) => ({
        title: feature.title,
        path: `${docsDir}/features/${feature.domain}/${feature.name}.md`,
